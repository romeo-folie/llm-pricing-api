...
// ─── Error boundary ────────────────────────────────────────────────────────────
// Catches WebGL context failures (GPU blocklist, driver crash, etc.) and falls
// back to the SVG HeroScene so the page never crashes.
...
class HeroErrorBoundary extends Component<{ children: ReactNode }, EBState> {
  ...
  render() {
    if (this.state.crashed) return <HeroScene />
...
function Model() {
  ...
  // Clone the cached scene so each mount gets an independent Three.js object.
  // Without this, remounting (client-nav, React Strict Mode) re-attaches the
  // same object and Three.js throws "object already has a parent".
  return (
    <Center>
 ...
 *   - HeroErrorBoundary catches WebGL failures and falls back to SVG HeroScene
 */
      ...
      <HeroErrorBoundary>
        ...
        >
          <Suspense fallback={null}>
            ...
            {/* Key light — castShadow removed: no mesh sets receiveShadow,
                so the shadow map was computed but never applied. */}
